...
// 明細情報を取得
$stmtDetail = $pdo->prepare("SELECT * FROM invoice_details WHERE invoice_id = :invoice_id ORDER BY order_index");
$stmtDetail->bindValue(':invoice_id', $id, PDO::PARAM_INT);
$stmtDetail->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  // バリデーション実行
  $errors = validateForm($formData, $rules, [], $pdo, 'quotes', $id);
  ...
  if (empty($errors)) {
    ...
    try {
      ...
      $stmt->execute([
        ...
        ':tax' => $tax,
        ':total_amount' => $total_amount,
        ':status' => $formData['status'] ?? '未払い',
        ':id' => $id
      ]);

      // 明細更新
      $pdo->prepare("DELETE FROM invoice_details WHERE invoice_id = :i_id")->execute([':i_id' => $id]);

      $stmtDetail = $pdo->prepare("
        INSERT INTO invoice_details
        (invoice_id, item_id, item_name, unit_price, cost_type, quantity, line_total, order_index)
        VALUES (:i_id, :item_id, :item_name, :unit_price, :cost_type, :quantity, :line_total, :order_index)
      ");
...
<div class="flex flex-1 relative">
  ...
  <main class="flex-1 p-6 bg-gray-50 w-full">
    <div class="max-w-3xl mx-auto bg-white shadow-xl rounded-xl p-8">
      <h1 class="text-2xl font-bold text-gray-900 mb-6 text-center">請求書編集画面</h1>
      ...
      <form method="POST" action="edit.php? id=<?= htmlspecialchars($id) ?>"
        x-data="invoiceForm(<?= htmlspecialchars(json_encode($formData)) ?>, <?= htmlspecialchars(json_encode($itemsToDisplay)) ?>, <?= htmlspecialchars(json_encode($products)) ?>)"
...
<script>
function invoiceForm(initialForm, initialItems, products) {
  return {
    form: {
      ...initialForm,
