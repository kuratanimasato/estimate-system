...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $rules = [
    ...
    'received_date' => [
      ...
      'date' => true,
      'message' => [
        'required' => '受領日は必ず入力してください。',
        'date' => '受領日の日付形式が正しくありません。'
      ]
    ],
    'remarks' => [
      'nullable' => true,
  ...
  // バリデーションがOKならDB登録
  $errors = validateForm($formData, $rules, [], $pdo, 'recept');
  // 明細チェック
  if (empty($items)) {
    ...
    foreach ($items as $index => $item) {
      ...
      if (!in_array($item['cost_type'], ['月額利用料', '初期費用', '機材費'])) {
        $errors['items'][$index]['cost_type'] = '費用区分が不正です。';
      }
    }
  ...
  if (empty($errors)) {
    ...
    try {
      ...
      $stmt = $pdo->prepare("
                  ...
                  payment_method, issue_date,received_date, subtotal, tax, total_amount, remarks,  created_at)
                  VALUES
                  (:document_type, :customer_id, :customer_name, :customer_email, :sales_rep_id, :sales_rep_name,
                  :payment_method, :issue_date, :received_date, :subtotal, :tax, :total_amount, :remarks, NOW())
              ");
      $stmt->execute(array_intersect_key($formData, array_flip([
        'document_type',
        'customer_id',
        'customer_name',
        'customer_email',
      ...
      $stmtDetail = $pdo->prepare("
                ...
                (receipt_id, item_id, item_name, unit_price, cost_type, quantity, line_total, order_index, created_at, updated_at)
                VALUES
                (:receipt_id, :item_id, :item_name, :unit_price, :cost_type, :quantity, :line_total, :order_index, NOW(), NOW())
            ");
      foreach ($items as $index => $item) {
      ...
      $pdo->rollBack();
      $errors['general'] = 'データベースエラー: ' . htmlspecialchars($e->getMessage());
...
<div class="flex flex-1 relative">
  ...
  <main class="flex-1 p-6 bg-gray-50 w-full">
    <div class="max-w-3xl mx-auto bg-white shadow-xl rounded-xl p-8">
      <h1 class="text-2xl font-bold text-gray-900 mb-6 text-center">新規領収書作成</h1>
