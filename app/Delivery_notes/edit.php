...
// 明細情報を取得
$stmtDetail = $pdo->prepare("SELECT * FROM delivery_note_details WHERE delivery_note_id = :delivery_note_id ");
$stmtDetail->bindValue(':delivery_note_id', $id, PDO::PARAM_INT);
$stmtDetail->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  // バリデーション実行
  $errors = validateForm($formData, $rules, [], $pdo, 'delivery', $id);
  ...
  // エラーがなければ更新
  $errors = validateForm($formData, $rules, [], $pdo, 'delivery');
  if (empty($errors)) {
    $subtotal = 0;
    foreach ($items as $it) {
    ...
    try {
      ...
      $stmt->execute([
        ...
        ':tax' => $tax,
        ':total_amount' => $total_amount,
        ':status' => $formData['status'] ?? '未送付',
        ':id' => $id
      ]);

      // 明細更新
      $pdo->prepare("DELETE FROM delivery_note_details WHERE delivery_note_id = :delivery_id")->execute([':delivery_id' => $id]);

      $stmtDetail = $pdo->prepare("
        INSERT INTO delivery_note_details
        (delivery_note_id, item_id, item_name, unit_price, cost_type, quantity, line_total)
        VALUES (:delivery_note_id, :item_id, :item_name, :unit_price, :cost_type, :quantity, :line_total)
      ");
...
<div class="flex flex-1 relative">
  ...
  <main class="flex-1 p-6 bg-gray-50 w-full">
    <div class="max-w-3xl mx-auto bg-white shadow-xl rounded-xl p-8">
      <h1 class="text-2xl font-bold text-gray-900 mb-6 text-center">納品書編集画面</h1>
      ...
      <form method="POST" action="edit.php? id=<?= htmlspecialchars($id) ?>"
        x-data="deliveryForm(<?= htmlspecialchars(json_encode($formData)) ?>, <?= htmlspecialchars(json_encode($itemsToDisplay)) ?>, <?= htmlspecialchars(json_encode($products)) ?>)"
        ...
        <div class="mb-4">
          ...
          <select id="sales_rep_id" name="sales_rep_id" x-model.number="form.sales_rep_id" @change="updateSalesRep"
            ...
            <?php foreach ($sales_reps as $sales_rep): ?>
              <option value="<?= $sales_rep['id'] ?>" data-name="<?= htmlspecialchars($sales_rep['name']) ?>">
                <?= htmlspecialchars($sales_rep['name']) ?>
              </option>
            <?php endforeach; ?>
          </select>
...
<script>
  function deliveryForm(initialForm, initialItems, products) {
    return {
      form: {
        ...initialForm,
