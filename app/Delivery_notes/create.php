...
$formData = [
  'document_type' => '納品書', // デフォルトを納品書に設定 (必要に応じて変更)
  'customer_id' => '',
  'customer_name' => '',
  'customer_email' => '',
  'sales_rep_id' => '',
...
// 明細データ取得
$stmtItems = $pdo->prepare("SELECT * FROM delivery_note_details WHERE  delivery_note_id = :delivery_note_id ORDER BY order_index");
    ...
    try {
      ...
      $stmt = $pdo->prepare("
                INSERT INTO delivery_notes
                    ...
                    issue_date, expiration_date, subtotal, tax, total_amount, status, created_at)
                VALUES
                    (:document_type, :customer_id, :customer_name, :customer_email, :sales_rep_id, :sales_rep_name,
                    :issue_date, :expiration_date, :subtotal, :tax, :total_amount, :status, NOW())
            ");
      ...
      $delivery_note_id = $pdo->lastInsertId();

      // 2. delivery_note_details 登録
...
<div class="flex flex-1 relative">
  ...
  <main class="flex-1 p-6 bg-gray-50 w-full">
    <div class="max-w-3xl mx-auto bg-white shadow-xl rounded-xl p-8">
      <h1 class="text-2xl font-bold text-gray-900 mb-6 text-center">新規納品書作成</h1>
