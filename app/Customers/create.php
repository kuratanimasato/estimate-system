...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  // バリデーションルールの定義
  $errors = validateForm($formData, $rules, [], $pdo, 'customers');
  // エラーがなければデータベースに保存
  if (empty($errors)) {
    try {
      // トランザクション開始
      ...
      // プリペアドステートメントでデータを挿入
      $stmt = $pdo->prepare("INSERT INTO customers (company_name, email, created_at, updated_at) VALUES (:company_name, :email, NOW(), NOW())");
      $stmt->execute([
      ...
      // データベースエラーが発生した場合
      $pdo->rollBack();
      $errors[] = 'データベースエラーが発生しました。時間を置いて再度お試しください。: ';
      error_log('Database Error: ' . $e->getMessage());
    }
  }
}
$company_name_value = htmlspecialchars($formData['company_name']);
$email_value = htmlspecialchars($formData['email']);
?>
<?php
