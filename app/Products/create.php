...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  // バリデーション
  $errors = validateForm($formData, $rules, [], $pdo, 'items');

  if (empty($errors)) {
    try {
      $pdo->beginTransaction();
      $stmt = $pdo->prepare("
        INSERT INTO items (item_name, unit_price, cost_type, created_at, updated_at) 
        VALUES (:item_name, :unit_price, :cost_type, NOW(), NOW())
      ");
      $stmt->execute([
      ...
      $_SESSION['flash_message'] = '商品を登録しました。';
      ...
      <div class="w-full bg-white shadow-xl rounded-xl p-8">
        ...
        <form method="POST" action="create.php">
          ...
          <div class="flex justify-end space-x-4 pt-4">
            ...
            <button type="submit"
              class="py-2 px-6 rounded-lg text-white bg-indigo-600 hover:bg-indigo-700">商品を登録</button>
          </div>
        </form>
      </div>
    </div>
