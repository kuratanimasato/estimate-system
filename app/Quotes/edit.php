...
// 明細情報を取得
$stmtDetail = $pdo->prepare("SELECT * FROM quote_details WHERE quote_id = :quote_id ORDER BY order_index");
$stmtDetail->bindValue(':quote_id', $id, PDO::PARAM_INT);
$stmtDetail->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  // バリデーション実行
  $errors = validateForm($formData, $rules, [], $pdo, 'quote', $id);
  ...
  if (empty($errors)) {
    ...
    try {
      ...
      $stmt->execute([
        ...
        ':tax' => $tax,
        ':total_amount' => $total_amount,
        ':is_copy' => $formData['is_copy'] ?? 0,
        ':status' => $formData['status'] ?? '作成中',
        ':id' => $id
      ]);

      // 明細更新
      $pdo->prepare("DELETE FROM quote_details WHERE quote_id = :qid")->execute([':qid' => $id]);

      $stmtDetail = $pdo->prepare("
        INSERT INTO quote_details
        (quote_id, item_id, item_name, unit_price, cost_type, quantity, line_total, order_index)
        VALUES (:qid, :item_id, :item_name, :unit_price, :cost_type, :quantity, :line_total, :order_index)
      ");
